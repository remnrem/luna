...
if ( ! is.null( factors ) ) { 
 ...
 df.meta <- data.frame( rep( variables , each = dim(df.meta)[1] ) , 
...
if ( ! is.null( factors ) ) { 
  df <- setDF( dcast( setDT( df ) , 
               as.formula( paste( c( paste( c("ID" , rows ) , sep="+", collapse="+" ) , 
	       		      "~" , 
			      paste( factors , sep="+", collapse="+" ) ) , collapse = " " ) ) , 
               value.var = variables ) )
...
if ( is.null( variables ) ) { idx <- rep( T , dim(df.meta)[1] ) 
} else { idx <- df.meta$BASE %in% variables } 
